...
## 第七部分：从 Skill 到 MCP Tool：把经验发布成标准化工具

第六部分解决的是“同一个 Skill 如何在不同 Coding Agent 中落地”。但如果我们希望这个能力不只是被不同客户端读取，而是能以统一接口被发现、传参和调用，就需要再往前走一步：把稳定的 Skill 封装成 MCP Tool。
...
这里继续使用第六部分的 `code-review` Skill。它适合作为 MCP 示例，原因很简单：
